...
contract('ERC20', function (accounts) {
    ...
    it('vérifie si un approve est bien effectué', async function () {
        ...
        await this.ERC20Instance.approve(spender, tokenAmount, {from:owner});

        let allowanceSpenderAfterApprove = await this.ERC20Instance.allowance(owner, spender);

        expect(allowanceSpenderAfterApprove).to.be.bignumber.equal(allowanceSpenderBeforeApprove.add(tokenAmount));
    });

    it('vérifie si un transfert est bien effectué', async function () {
        let balanceOwnerBeforeTransfer = await this.ERC20Instance.balanceOf(owner);
        let balanceRecipientBeforeTransfer = await this.ERC20Instance.balanceOf(recipient);
        let tokenAmount = new BN(10);
        
        await this.ERC20Instance.approve(spender, tokenAmount, {from: owner});
        await this.ERC20Instance.transferFrom(owner, recipient, tokenAmount, {from: spender});

        let balanceOwnerAfterTransfer = await this.ERC20Instance.balanceOf(owner);
        let balanceRecipientAfterTransfer = await this.ERC20Instance.balanceOf(recipient);

        expect(balanceOwnerAfterTransfer).to.be.bignumber.equal(balanceOwnerBeforeTransfer.sub(tokenAmount));
        expect(balanceRecipientAfterTransfer).to.be.bignumber.equal(balanceRecipientBeforeTransfer.add(tokenAmount));
    });
});